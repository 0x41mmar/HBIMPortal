...
var dataApi = 'http://vis.hbim.org'; //'http://localhost:8080';
var contentAPI = 'http://api.hbim.org/';
var APIUrl = dataApi + '/api'; //for data

function toggleIn() {$('.nav-side-menu').fadeOut(0); $('.nav-side-menu').css('z-index',10); $('.nav-side-menu').fadeIn(200)}
function toggleOut() {$('.nav-side-menu').fadeOut(200); setTimeout(function(){$('.nav-side-menu').fadeIn(0);$('.nav-side-menu').css('z-index',1);},210)}
	
function loadSideBar(disabledControls) {	
	$('#toggle').click(function() {
		$('.nav-side-menu').css('z-index')==10? toggleOut():toggleIn();
	})
	
	var models = viewModels;
	models.forEach(function(model,i) {
		$('#models').append('<li><a href="model.html?docID='+i+'">'+model.name+'</a></li>');
	});

	//Here see what doc types are there, and create a menu item for any that's not reports/analysis
	
	documents.reports.forEach(function(report, i) {
		if (report.type=='default')
			return;
		
		var menu = '#'+report.type;
		$('<li value="'+i+'">'+report.title+'</li>').appendTo(menu).click(function() {
			//UI stuff
	...
	// add bindings for analysis items
	
	//check if we have a real-time view
	if (!(!nextState || Object.keys(nextState).length==0))
		$('li[component="component_realtime"]').show().click(function() {viewer.restoreState(nextState)})
	
	//Now activate the other buttons
	$('.menu-content>li').each(function(i,item) {
		if (!item.attributes['component'])
			return;
		if (item.attributes['component'].value == "component_realtime")
			return;
		$(item).click(function() {loadComponent($('#'+item.attributes['component'].value))})
	});
	disabledControls.forEach(function(item) {
		$('li[component="component_'+item+'"]').unbind('click')
		$('li[component="component_'+item+'"]').css('background','rgba(193,193,193,0.5)');
	})
}

function loadComponent(component) {
	//hide sidebar if visible
	$('#navPanel').slideUp(500);
	
	//empty sidebar
	$('#navPanel').html('')
	
	//load up a new component inside it.
	$('#navPanel').append(component)
	
	//show sidebar now
	$('#navPanel').slideDown(500);
...
function loadReport(reportId) {
	//load report component into sidebar
	loadComponent($('#component_reports'))
	
	//load report file into sidepanel, iframe (or whatever)
	$('#reportDoc')[0].src = './reports/' + documents.reports[reportId].path;
	
	//if there's an images report, load that too
	if (documents.reports[reportId].imagesDoc)
		$('#imagesDoc')[0].src = './reports/' + documents.reports[reportId].imagesDoc;
	//show reportPanel
	$('#navPanel').slideDown(500);

	//load report states into stateManager, checked on panel load
	activeReport = reportId;
		//check if panel open, and if so close and re-open it
	
	//load default state into viewer
	if(documents.reports[reportId].states)
		viewer.restoreState(documents.reports[reportId].states[0])
}

function loadAnalysis(id) {
	//load report file into sidepanel, iframe (or whatever)
	$('#reportDoc')[0].src = './reports/' + documents.analysis[id].path;
	
	//show reportPanel
	$('#navPanel').slideDown(500);

	//load default state into viewer
	viewer.restoreState(documents.analysis[id].states[0])
}

function parseTimeline() {
	//timeline is an array of objects
	//convert obj into markup, add into timeline component on page
	
	var templates = {
		date: {markup:'<div class="separator text-muted"><time>{0}</time></div>',
				map:['date']},
		event: {markup:'<article class="panel panel-info panel-outline"><div class="panel-heading icon"><i class="glyphicon glyphicon-{0}"></i></div><div class="panel-body">{1}</div></article>',
				map:['icon','text']},
		article: {markup: '<article class="panel panel-primary"><div class="panel-heading icon"><i class="glyphicon glyphicon-plus"></i></div><div class="panel-heading"><h2 class="panel-title">{0}</h2></div><div class="panel-body">{1}</div><div class="panel-footer"><small>{2}</small></div></article>',
				map:['title','body','footer']}
	}
	
	var div = $('#component_timeline>.timeline')
	for (var i in timeline) {
		var markup = templates[timeline[i].type].markup;
		var map = templates[timeline[i].type].map;
		for (var m in map)
			markup = markup.replace('{'+m+'}',timeline[i][map[m]])
		div.append(markup);
	}
}

function loadWidget(path, element) {
	//have we loaded this widget before? just move it here
	var loaded = false;
	$('link[rel="import"').each(function(i,item) {
		if (item.attributes['href'].value == path && item.id.length>0) {
			$('#'+item.id.replace('import_','')).appendTo(element);
			loaded = true;
		}
	})
	if (loaded) return true;
	
	//load a widget and append into an element
	var link = document.createElement('link');
	link.rel = 'import';
	link.href = path;
	link.onload = function(e) {
		var div = this.import.querySelector('div');
		
		//record that we loaded this
		this.id = 'import_' + div.id;
		element.appendChild(this.import.querySelector('div')); //TypeError when 404
	};
...
function loadNextState() {
	...
	setTimeout(function(){
		try {
			...
			console.log("[-] Attempted to restore state before loading complete, waiting 2 seconds");
		...
		},3000); //very temporary hack, I promise  //Update January 2018: I hate you.
}

function loadExtensions() {	
	viewer.loadExtension("Viewing.Extension.Markup3D");
	markups3D = viewer.getExtension("Viewing.Extension.Markup3D");
	//viewer.loadExtension("Viewing.Extension.StateManager");
	viewer.loadExtension('Viewing.Extension.StateManager', {apiUrl: contentAPI+'/state',model: {_id: defaultIndex}})
